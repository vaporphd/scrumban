...
- Use `--no-verify` to bypass hooks. If a hook fails, fix the underlying cause. The **known host-env trap**: pre-push pytest fails with `socket.gaierror` resolving `postgres:5432` because `backend/.env` uses the compose hostname `postgres` (only resolves inside the compose network). Canonical fix (post-#67): `cp backend/.env.local.example backend/.env.local` once. Pydantic-settings loads `.env.local` after `.env` and the host-shell `DATABASE__URL=...@localhost:5432/...` wins inside the pre-push pytest subprocess. The old per-push `DATABASE__URL=... git push ...` env prefix still works as an emergency fallback but should not be the normal path. Never use `--no-verify` as a shortcut.
